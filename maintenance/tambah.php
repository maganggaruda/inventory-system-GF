...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Gabungkan tanggal dari form dengan jam/menit/detik saat ini
    $tanggal_lengkap = $input_tanggal . ' ' . date('H:i:s');

    // Validasi sederhana input wajib
    if (!empty($id_komponen) && !empty($tindakan) && !empty($status)) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            // Otomatis update kondisi komponen jika status 'Selesai'
            if ($status == 'Selesai') {
        ...
        $error = "Mohon lengkapi seluruh bidang yang wajib diisi (*)!";
    }
}

// Fetch daftar komponen beserta nama & kode mesin
